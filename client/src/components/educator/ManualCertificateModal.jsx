...
const ManualCertificateModal = ({ isOpen, onClose, student, courseId, courseTitle }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const { data } = await api.post('/certificate/manual-issue', formData, {
                headers: { 
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#0C132B]/80 backdrop-blur-xl animate-in fade-in duration-500">
            <div className="bg-white w-full max-w-xl rounded-[3.5rem] shadow-2xl shadow-emerald-500/20 border border-emerald-100 overflow-hidden relative animate-in zoom-in-95 duration-500">
                <button 
                    onClick={onClose}
                ...
                <div className="p-12 md:p-16">
                    <div className="flex flex-col items-center text-center mb-12">
                        <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-[2.5rem] flex items-center justify-center mb-8 shadow-inner">
                            <Upload size={32} />
                        </div>
                    ...
                    <div className="bg-gray-50/50 rounded-[2rem] p-8 border border-gray-100 mb-10">
                        <div className="flex items-center gap-4 mb-4">
                            <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest px-3 py-1 bg-white rounded-lg border border-emerald-100">Target Module</span>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        <div className="relative group">
                            <input 
                                ...
                                required
                            />
                            <div className="bg-white border-2 border-dashed border-emerald-200 rounded-[2rem] p-12 text-center group-hover:border-emerald-500 group-hover:bg-emerald-50/10 transition-all">
                                {file ? (
                                    ...
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Inject PDF/Image Asset</span>
                        ...
                        <button 
                            type="submit"
                            disabled={loading}
                            className="w-full bg-[#0C132B] text-white py-6 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-2xl shadow-emerald-500/10 disabled:opacity-50"
                        >
