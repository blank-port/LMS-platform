    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center h-[60vh] instructor-theme">
            <div className="relative w-24 h-24">
                ...
                <div className="absolute inset-0 border-[6px] border-t-blue-600 rounded-full animate-spin shadow-lg"></div>
            </div>
            <p className="mt-8 text-[11px] font-black uppercase tracking-[0.4em] text-blue-600 animate-pulse">Synchronizing Studio Intel...</p>
        </div>
    );

    const stats = [
        { label: 'Subjects Taught', value: dashboardData?.totalSubjects || 0, icon: BookOpen, color: 'text-blue-600', bg: 'bg-blue-50', trend: 'Curriculum' },
        { label: 'Total Students', value: dashboardData?.totalEnrollments || 0, icon: Users, color: 'text-cyan-600', bg: 'bg-cyan-50', trend: 'Global Reach' },
        { label: 'Gross Enrolled', value: `${currency}${dashboardData?.todayEnrolledAmount || 0}`, icon: CreditCard, color: 'text-emerald-600', bg: 'bg-emerald-50', trend: 'Daily Volume' },
        { label: 'Total Capital', value: `${currency}${dashboardData?.totalRevenue || 0}`, icon: Wallet, color: 'text-indigo-600', bg: 'bg-indigo-50', trend: 'Fiscal State' },
        { label: 'Studio Balance', value: `${currency}${dashboardData?.todayRevenue || 0}`, icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50', trend: 'Current Ops' },
        { label: 'Monthly Growth', value: `${currency}${dashboardData?.thisMonthRevenue || 0}`, icon: Calendar, color: 'text-rose-600', bg: 'bg-rose-50', trend: 'Expansion' },
    ];

    return (
        <div className="max-w-[1600px] mx-auto space-y-12 animate-fade-in pb-20 instructor-theme">
            {/* Studio Header */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/30 group">
                ...
                <div className="bg-slate-900 p-12 rounded-[3.5rem] shadow-[0_40px_80px_rgba(15,23,42,0.2)] relative overflow-hidden group">
                    ...
                    <div className="flex items-center justify-between mb-12 relative z-10">
                        <div>
                            <h2 className="text-2xl font-black text-white tracking-tighter leading-none mb-2">Engagement Pulse</h2>
                    ...
                    <div className="mt-10 flex items-center justify-between relative z-10 border-t border-white/5 pt-10">
                        <div className="space-y-1">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest opacity-60">Settlement Target</p>
                            <p className="text-3xl font-black text-white tracking-tighter">{currency}0.00</p>
                        </div>
                        <button className="px-10 h-16 bg-blue-600 text-white rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.3em] hover:bg-blue-500 transition-all shadow-2xl shadow-blue-500/40 hover:-translate-y-1 active:scale-95 leading-none">Execute Payout</button>
                    </div>
                </div>
            </div>
